...
xapeq2(ces,i)$(ene0(i) ne 0)..
      xap(ces,i)*(sum(ef,ctax(ces,i,ef))+pa(ces))**sigmaene(i) =e= aene(ces,i)*ene(i)*(pene(i)**sigmaene(i)) ;

  xap.fx(ces,i)$(  xap0(ces,i) eq 0) = 0 ;

peneeq(j).. pene(j)=e= sum(ces, aene(ces,j)*
					(sum(ef,ctax(ces,j,ef))+pa(ces)*(1+sum(g,tcp(ces,j,g))))
					**(1-sigmaene(j)));
...
gyeq(g)..
   gy(g) =e= sum(i,tp(i,g)*px(i)*xp(i))
       ...
       + sum((ces,i), sum(ef,ctax(ces,i,ef))*xap(ces,i))
       ;
