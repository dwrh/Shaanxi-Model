...
pxeq(i)..
*   (px(i)+rac("co2",i)*sum(j,tfr(j)*pr(j)*rd(j)/(1+tfr(j))/xp(i))$ifrr)**(1-sigmap(i)) =e= and(i)*pnd(i)**(1-sigmap(i)) + ava(i)*pva(i)**(1-sigmap(i)) ;
...
*   pp(i) =e= (1+tp(i)+sum(ef,tef(ef,i)*efi(ef,i))/xp(i))*px(i) ;

tpeq(i,g)..
   tp(i,g) =e= tp0(i,g) ;
*   tp(i) =e= tp0(i) - (recycle*ry*(rd(i)/sum(ii,rd(i)))/(px(i)*xp(i)))$xp0(i) ;
...
pndeq(j)..
   pnd(j) =e= sum(k,a(k,j)*(1+sum(g,tcp(k,j,g)))*pa(k)) ;
...
rdeq(i)$(rd0(i) ne 0)..
*   lambdar(i)*rd(i)*(pr(i)*(1+tfr(i)))**sigmav(i) =e= ar(i)*va(i)*(lambdar(i)*pva(i))**sigmav(i) ;
...
*          +  sum(lt,xth(lt,h)*ty(lt)) + plev*(gtrh(h)+atrh(h)*ry*rsc) + sum(hh,htrh(h,hh)) + er(r)*wtrh(h) ;
...
*# old emission block
$ontext
   efieq(ef,i)$efi0(ef,i).. efi(ef,i) =e= xp(i)*emit(i,ef) ;
*   efieq(ef,i)$efi0(ef,i).. efi(ef,i) =e= mue(ef)*(rd(i)-offset(i)) ;

        efi.fx(ef,i)$(efi0(ef,i) eq 0) = 0 ;


   efheq(ef,h)$efh0(ef,h).. efh(ef,h) =e= sum(k,sum(i$mapik(i,k),xac(k,h)*emit(i,ef))) ;
*   efheq(ef,h)$efh0(ef,h).. efh(ef,h) =e= sum(k,sum(i$mapik(i,k),xac(k,h)*EF_GHG(i,"GHG_Total"))) ;
*   efheq(ef,h)$efh0(ef,h).. efh(ef,h) =e= mue(ef)*sum(k,lambdac(ef,k,h)*xac(k,h)) ;
...
model PANDA /
   ...
   , efieq, efheq, efteq, efcapeq, efcieq, sfreq
*   pefxeq, cefxeq
   , tpeq
*       , bop
*, era
